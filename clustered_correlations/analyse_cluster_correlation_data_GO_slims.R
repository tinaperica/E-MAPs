...
load( paste0( "preprocessed_correlations_GO_slim/", "GO_slims_pearson_complete_corr_", cluster_number, ".RData" ) )
outputfilename <- paste0("Output/GO_slims_pearson_complete/GO_slim_cluster_", cluster_number, "_corr_of_corr.RData")

cluster <- corr_lst[["cluster"]]
mutants <- as.character(corr_lst[["mutants"]])
###
mutant_partner_corr_of_corr <- data.frame()
for (p in seq_len( nrow(corr_lst[["table_of_pairs"]]) ) ) {
  gene_A <- as.character( corr_lst[["table_of_pairs"]][["gene_A"]][p] )
  gene_B <- as.character( corr_lst[["table_of_pairs"]][["gene_B"]][p] ) 
  mut_correlations_df <- corr_lst[["correlations"]][
    corr_lst[["correlations"]][["gene1"]] == gene_A, ]
  temp_mut_correlations_df <- corr_lst[["correlations"]][
    corr_lst[["correlations"]][["gene2"]] == gene_A, ]
  ...
  partner_correlations_df <- corr_lst[["correlations"]][
    corr_lst[["correlations"]][["gene1"]] == gene_B, ]
  temp_partner_correlations_df <- corr_lst[["correlations"]][
    corr_lst[["correlations"]][["gene2"]] == gene_B, ]
  ...
  if ( (nrow(mut_correlations_df) > 3 ) & ( nrow(partner_correlations_df) > 3) ) { 
    ...
    if ( nrow(merge_for_corr) > 3 ) {
      names(merge_for_corr) <- expression(uber_gene, cluster, cluster_method, geneA, geneA.corr, geneB, geneB.corr)
  ...
  temp.corr_of_corr <- cbind(temp.corr_of_corr, data.frame("geneA" = geneA, "geneB" = geneB))
