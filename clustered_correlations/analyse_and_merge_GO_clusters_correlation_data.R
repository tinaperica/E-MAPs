...
step_size <- as.numeric( Sys.getenv( "SGE_TASK_STEPSIZE" )) - 1  ## 551
...
GO_clusters <- as.character(unique(merged_correlations_df[["GO_cluster"]]))
for (i in seq_along(GO_clusters)) {
  mutant_partner_corr_of_corr <- data.frame()
  GO_cluster <- GO_clusters[i]
  GO_merged_correlations_df <- merged_correlations_df[merged_correlations_df[["GO_cluster"]] == GO_cluster, ]
  for (p in  first_pair:last_pair ) {
    mutant <- as.character( table_of_pairs[["mutant"]][p] )
    partner <- as.character( table_of_pairs[["partner"]][p] ) 
    mut_correlations_df <- GO_merged_correlations_df[
      GO_merged_correlations_df[["gene1"]] == mutant, ]
    temp_mut_correlations_df <- GO_merged_correlations_df[
      GO_merged_correlations_df[["gene2"]] == mutant, ]
    ...
    partner_correlations_df <- GO_merged_correlations_df[
      GO_merged_correlations_df[["gene1"]] == partner, ]
    temp_partner_correlations_df <- GO_merged_correlations_df[
      GO_merged_correlations_df[["gene2"]] == partner, ]
  ...
  final.list<-list()
  final.list[["cluster"]] <- GO_cluster
  final.list[["cluster_number"]] <- 45+i
  final.list[["corr_of_corr_df"]] <- mutant_partner_corr_of_corr
  
  outputfilename <- paste0(Sys.Date(), "_cluster_", 45+i, "_", first_pair, "_corr_of_corr.RData")
