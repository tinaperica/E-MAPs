...
setwd("~/Documents/GSP1/E_MAP_data/June2016_analysis/clustered_correlations/RData/")
...
for (d in seq_along(dir_list)) {
  ...
  if (length(all_correlations[1,]) > 0) {
    ...
    write.table(all_correlations, file = paste0(dir, "_merged_correlations.txt"), quote = F, sep = "\t", row.names = F)
