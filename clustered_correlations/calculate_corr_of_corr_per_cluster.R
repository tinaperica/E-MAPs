...
task_n <- 1
#inputfilename <- paste0("info_for_mut_partner_corr_of_corr_per_cluster/", task_n, "_task_info.RData")
inputfilename <- paste0("~/Box Sync/kortemmelab/home/tina/Gsp1/E-MAP_analysis_backup_data/info_for_mut_partner_corr_of_corr_per_cluster/", 
                        task_n, "_task_info.RData")
...
corr_input_file <- paste0("~/Box Sync/kortemmelab/home/tina/Gsp1/E-MAP_analysis_backup_data/March2018_preprocessed_correlations/corr_w_random__", cluster_n, ".RData")
...
for (p in 1:100) {
  ...
  corr_of_corr <- rbind(corr_of_corr, data.frame( 
    geneA, geneB, cluster, 
   # "pair" = paste0(geneA, geneB), 
   "corr_of_corr_type" = c("corr_of_corr", "random_corr_of_corr", "random_corr_of_corr_2", 
...
##### the p value correactions don't make sense anymore in this script because it is doing one cluster at a time
### but I forgot to delete it
### make sure not to use these p values, calculate them later
...
#### don't use the corrected p values from these files
outputfilename <- paste0("20180409_correlations_of_correlations_mut_partner_pairs_only_", cluster_n, "_", task_n,  ".RData")
outdir <- paste0("Output/corr_of_corr_mut_per_cluster/")
