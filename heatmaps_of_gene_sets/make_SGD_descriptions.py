...
SGD_to_merge.to_csv('datasets/SGD_descriptions_all.txt', sep='\t', na_rep='', index=False)
...
df = pd.merge(df, pd.read_csv('datasets/SGD_missing_descriptions.txt', sep='\t'), how = 'outer')

# drop if description is empty (won't add any information)
SGD_descriptions = df.loc[~pd.isnull(df.description)]

# save dataframe
SGD_descriptions.to_csv('datasets/SGD_descriptions_library_genes.txt', sep='\t', na_rep='', index=False)
