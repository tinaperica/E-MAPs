...
setwd("/Users/tina/Documents/GSP1/E_MAP_data/June2016_analysis/GIA_analysis/")
mean.na<-function (x) {
  mean(x, na.rm=T)
}
min.na<-function (x) {
...
e.map<-read.delim("avg_merged_June2016_screen_for_Gia.txt", head=T, sep="\t")
complex.annotation<-read.delim("uncurated_complex_annotation.txt", head=T)
#complex.annotation<-read.delim("GTPases_annotation.txt", head=T)
