...
library(factoextra)
library(heatmaply)
# need to run the following one time to save heatmaply files:
...
spitzemap_to_corr <-
  spitzemap %>% 
  filter(interaction_network %in% c('gsp1_pEMAP','NxN_SGA', 'ExN_SGA'),
         !grepl('tsa', array_strain_id)) %>%
  filter(query_ORF %in% partners$ORF) %>%
  select(query_allele_name, array_ORF, score)
...
dd <- as.dist((1-cormat)/2) # create a distance matrix, corr of "1" has dist 0 and "-1" has dist 2
hc <- hclust(dd)
...
ggsave('corr.pdf', height = 20, width = 20)
```
...
heatmap(mat, na.rm = FALSE, keep.demdrp)
