...
public class ColonyGrid {
	...
	protected void setUpGrid(ImagePlus im, ResultsTablePlus rt)
		{
	...
	private ImagePlus applyGridMask(ImagePlus im, int flag)
		{	//this flag is not currently used except to distinguish the two methods
	...
	protected double adjustGridTop(ResultsTablePlus rt, int w)
		...
		while ((tempNum < 3) & (stepNum < 10))
			{	//probably looked too high
		...
		//System.out.println("Old bottom row: "+numGoodCurrent+" New top row: "+numGoodAlt);
		if ((numGoodAlt>numGoodCurrent) & ((newPos-fullSpace)>0))
			{
			newPos=newPos-fullSpace;
			System.out.println("Adjusting grid top position");
			//System.out.println("Old bottom row: "+numGoodCurrent+" New top row: "+numGoodAlt);
			}
		return newPos;
		}
	...
	protected boolean isInCorrectRow(double x, double y, double boxT, double boxB)
		...
		if ((y>boxT)&(y<boxB))
			{	//Don't bother with the ovalRoi unless y is in the right range
