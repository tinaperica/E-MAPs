...
outputfilename <- paste0("20180921_", task_n, "_correlations.RData")
load(inputfilename)
load("20180921_emap_data_for_corr.RData")
...
for ( p in seq_along(task.info[["pairs"]][1,]) ) {
  ...
  ubermap.query1 <- ubermap[ubermap[["Gene_uniq"]] == query1, ]
  ubermap.query2 <- ubermap[ubermap[["Gene_uniq"]] == query2, ]
  merged.emap.data <- merge(ubermap.query1, ubermap.query2, by = "library.ORF")
  merged.emap.data <- merged.emap.data[complete.cases(merged.emap.data),]
  if (length(merged.emap.data[,1] > 500)) {
