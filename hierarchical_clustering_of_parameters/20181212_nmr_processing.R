...
nmr_raw_data <- as.tibble(read_delim('hierarchical_clustering_of_parameters/20181018_nmr_data.csv', delim = ','))
...
ggplot(data.to.plot, aes(reorder(gsp1_mutation, gamma2_percent), gamma2_percent, 
    ...
    xlab("Gsp1 mutant") +
    ylab("Percent in Closed Conformation") +
    ylim(0,100) +
    theme(text = element_text(size=30),
    axis.text.x = element_text(angle = 90, hjust = 1))
ggsave("hierarchical_clustering_of_parameters/NMR_gamma2_percents.pdf", height = 10, width = 12)
