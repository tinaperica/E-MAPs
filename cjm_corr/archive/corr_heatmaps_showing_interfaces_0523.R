...
cormat <- corr.test(t(pmask*mat), use = 'pairwise', method = 'pearson')
dissim <- as.dist((1 - cormat$r)/2)
dissim2 <- as.matrix(dist(pmask*mat))
dissim2[is.na(dissim2)] <- max(dissim2, na.rm = T)
dissim2 <- as.dist(dissim2)

row_order <-
  cmdscale(dissim2, eig = T, k = 1) %>%
  `$`(point) %>%
  as_tibble(rownames = 'mutant') %>%
  arrange(V1) %>%
  pull(mutant)
hc.row <- hclust(dissim, method = "complete" )
hc.row <- rotate(hc.row, row_order)
# row_order <- hc.row$labels[hc.row$order]

# cols (partners)
cormat <- corr.test(pmask*mat, use = 'pairwise', method = 'pearson')
dissim <- as.dist((1 - cormat$r)/2)
# dissim <- dist(t(mat))
...
hc.col <- hclust(dissim, method = "complete")
